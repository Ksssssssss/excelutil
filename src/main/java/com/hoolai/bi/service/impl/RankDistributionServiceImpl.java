...
@Service
public class RankDistributionServiceImpl implements ReportService {
    ...
    public ExcelDatas produceDatas(String startDs, String endDs, int gameId) {
        ...
        List<RankDistribution> rankDistributionList = rankDistributionsMapper.queryDauRanks(gameId,startDs,endDs);
        result = rankDistributionList.stream().collect(Collectors.groupingBy(rankDistribution->rankDistribution.getDs()));
        return new RankDistributions(result,ReportType.DAU_RANK_DISTRIBUTION, ExtraType.DAU_RANK_DISTRIBUTION);
    }
}
